// routes/horarios.js (COMPLETAMENTE MODIFICADO)

const express = require("express");
const router = express.Router();
const db = require("../db"); // Asegúrate que db.js exporte el pool de 'mysql2/promise'
...
router.post("/horario", async (req, res) => {
  ...
  try {
    ...
    // 3. VALIDACIÓN DE GEOFENCE (¿El alumno está en el edificio correcto?)
    // Creamos un punto GPS (Longitud, Latitud)
...
  	 `;
    ...
    // SRID 4326 es el estándar para GPS
    ...
    const esta_dentro = geofenceResult[0].esta_dentro; // 1 (true) o 0 (false)
    ...
    // 5. VALIDACIÓN DE HORARIO (Si está dentro, revisamos si tiene clase)
    const sqlHorario = `
  	 	 SELECT 
  	 	 	 M.id AS id_materia, 
  	 	 	 M.nombre AS nombre_materia, 
  	 	 	 M.duracion_minutos,  -- <-- DATO NUEVO PARA EL CRONÓMETRO
  	 	 	 H.dia, 
  	 	 	 H.hora_inicio, 
  	 	 	 H.hora_fin
  	 	 FROM Horarios H
...
  	 `;
    ...
    // 6. RESPUESTA EXITOSA (Enviamos todo lo que Flutter necesita)
    res.status(200).json({
      usuario: id_usuario,
      id_materia: horario.id_materia,
      sub_clase: horario.duracion_minutos, // <-- DATO NUEVO PARA EL CRONÓMETRO
